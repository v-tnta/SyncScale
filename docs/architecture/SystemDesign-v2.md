...
graph LR
    ...
    CE -->|1. chrome.tabs.sendMessage| WA1
    WA1 -->|2. 一括データ登録| FS
    SP1 -->|見積もりラベル| FS
    SP2 -->|作業ログ| FS
    SP3 -->|コンディション| FS
...
- **[NEW ver0.1.4] Chrome拡張機能とのメッセージ連携**:
  - 拡張機能からの `postMessage` をリスニングし、ログイン状態に応じたスマートインポートを提供。
  - Firestoreの `writeBatch` を使用した `addTasksBatch` による、課題の一括重複排除・一括登録処理。

#### 🔌 Chrome拡張機能 (v1.1.0)
- LMS（manaba等）のページからタスク名と〆切を自動取得（スクレイピング）。
- **[NEW v1.1.0] セキュリティ・Manifest V3準拠の安全設計**:
  - 拡張機能内部における Firebase SDK および APIキー・認証情報の直接所持を**完全廃止**。
  - 取得データを稼働中のSyncScale Webアプリのアクティブなタブへ `chrome.tabs.sendMessage` を使って直接送信し、Webアプリを介してFirestoreに登録するセキュアな設計に刷新。
...
Firebaseを **Single Source of Truth（唯一の情報源）** とし、Webアプリとスマートフォンアプリが同一のFirestoreコレクション・同一のAuthユーザーを共有します。
**Chrome拡張機能については、セキュリティの最大化とManifest V3の厳格なポリシーに準拠するため、Firebase直接接続を完全に廃止し、Webアプリの動作中タブを介したメッセージ連携方式を採用しています。**
これにより：
- **セキュリティの最大化**: 拡張機能内に Firebase APIキーや認証トークンを保持・露出させるリスクを完全に排除します。
- **データ整合性**: 拡張機能が送信したデータは、Webアプリ側の安全なバリデーションと重複排除ロジックを経由してFirestoreに一括登録（`writeBatch`）されます。
- **ユーザー体験の向上**: Webアプリが未ログインの場合でも、拡張機能からデータを送信するとWebアプリ側で一時バッファに保留され、Googleログイン完了と同時にシームレスにデータがインポートされます。
...
#### 🆕 4.3.10 拡張機能連携と一括インポートモーダル

- **機能**: Chrome拡張機能から送信された課題データを受信し、重複排除を行った上で一括登録する。
- **UI要素（ログイン待ちモーダル）**:
  - タイトル「課題が見つかりました！」
  - 概要説明（取り込み可能な課題数、Googleログインの推奨）
  - 「Googleでログイン」ボタン（Google Authを起動）
  - 「今は取り込まない」ボタン
- **挙動**:
  - Webアプリ（`App.jsx`）がアクティブになると、`window.addEventListener('message')` で拡張機能からの課題データメッセージ（`SYNC_SCALE_IMPORT_TASKS`）を受信。
  - **ログイン済みの場合**: 即座にインポート処理を実行。
  - **未ログインの場合**: データを一時バッファ（`pendingImportTasks`）に格納し、最前面（z-index: 100）にログイン誘導モーダルを表示。ログイン完了検知後に自動でインポート処理を実行。
  - **重複排除処理**: `tasks`（現在取得しているタスク）内の `manabaAssignmentId` を参照し、すでに登録されている課題は自動スキップ。新規課題のみを抽出。
  - **一括登録処理**: `useTasks` フック経由で `taskService.addTasksBatch` を呼び出し、Firestoreの `writeBatch` により一括書き込み（アトミックな書き込みによりパフォーマンスと安定性を担保）。
