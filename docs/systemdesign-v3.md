...
| バージョン | 日付 | 概要 |
|---|---|---|
| `0.3.7` | 2026/06/17 | **締切前通知の設定を Web 版・Flutter Web 版でも表示。** これまでネイティブ専用だった「締切前に通知」設定を、React Web 版の設定パネルと Flutter Web ミラーの設定ダイアログにも表示。設定値は `userSettings/{uid}`（`notificationEnabled` / `notificationMinutesBefore`）に保存され、スマホアプリと共有される。Web では実際の通知が配信されないため「通知はスマートフォンアプリでのみ届く」旨を明記し、Web では OS 通知権限の要求を行わないよう調整。 |
