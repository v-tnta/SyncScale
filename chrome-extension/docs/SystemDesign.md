...
sequenceDiagram
    ...
    Note over CS: バックグラウンドで /ct/home_library_query をfetch<br/>取得したHTMLをパースして課題抽出

    CS->>SW: { action: "SCRAPE_RESULT", tasks: [...] }
    
    Note over SW: 既存タスクと重複チェック<br/>（manabaAssignmentIdで照合）
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === "SCRAPE_TASKS") {
    ...
    // 非同期処理でfetchとパースを行うため、即時実行関数を使用
    (async () => {
      try {
        const tasks = await scrapeTasks();
        console.log("[Content Script] 抽出結果:", tasks);
        // 結果をService Workerへ返す
    ...
    return true; // 非同期でsendResponseを呼ぶため true を返す
  }
});

/**
 * /ct/home_library_query をfetchして課題一覧を抽出する
 * @returns {Promise<Array<{manabaAssignmentId: string, manabaCourseId: string, courseName: string, title: string, type: string, deadline: string}>>}
 */
async function scrapeTasks() {
  const tasks = [];
  
  // 現在開いているmanabaのドメインを利用してURLを構築 (例: manaba.ibaraki.ac.jp)
  const url = `https://${window.location.hostname}/ct/home_library_query`;
  
  // 未提出課題ページをバックグラウンドで取得
  const res = await fetch(url);
  if (!res.ok) {
    throw new Error("課題ページの取得に失敗しました");
  }
  ...
  const rows = doc.querySelectorAll("table.stdlist tr:not(.title)");
...
async function registerTasks(userId, scrapedTasks) {
  // 既存タスクの manabaAssignmentId 一覧を取得（重複チェック用）
  ...
  for (const task of scrapedTasks) {
    // manabaAssignmentId が既に存在する場合はスキップ
...
<body>
  <div class="container">
    ...
    <section id="fetch-section">
      <p class="hint">manabaのページを開いた状態で押してください</p>
...
```javascript
{
  ...
  manabaAssignmentId: "course_1194265_project_1232862", // manaba上の課題ユニークID
  manabaCourseId: "course_1194265",                     // manaba上のコースID
  courseName: "卒業研究（T5099-J03B）",                 // コース名
  title: "プロジェクトその１B",                         // manabaから取得した課題名
  type: "プロジェクト",                                 // 課題のタイプ(レポート、小テストなど)
...
```
同一ユーザー（userId）かつ
同一 manabaAssignmentId（manabaAssignmentId）
→ すでに存在する場合はスキップ
```

> **補足**: 以前はタイトルで重複判定していたが、`manabaAssignmentId` (例: `course_1194265_project_1232862`) がURLから取得できるため、これをユニークキーとして重複判定を行うことで同名の別課題でも正しく処理されるようになった。
